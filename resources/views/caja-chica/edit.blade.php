                ...
                <div class="p-3">
                    <div class="row align-items-center justify-content-center">
                        <div class="sidebar-brand-text m-3">
                            <h4 class="font-weight-bold">EDITAR CAJA CHICA</h4>
                        </div>
        ...
        <form action="{{ route('caja-chica.update', $cajaChica) }}" method="POST">
            ...
            <div class="form-row m-2">
                <div class="form-group col-lg-6 col-md-6 col-xs-6 mb-4">
                    ...
                    <select id="proyecto" name="proyecto" class="form-control @error('proyecto') is-invalid @enderror">
                        ...
                        @foreach ($proyectos as $proyecto)
                            <option class="text-uppercase" value="{{ $proyecto->id }}" @selected($proyecto->id == $cajaChica->proyecto_id)>{{ $proyecto->nombre_proyecto }}</option>
                        @endforeach
                    </select>
                    @error('proyecto')
                ...
                <div class="form-group col-lg-6 col-md-6 col-xs-6 mb-4">
                    ...
                    <select id="autorizado_por" name="autorizado_por" class="form-control text-uppercase @error('autorizado_por') is-invalid @enderror">
                        ...
                        @foreach ($responsables as $responsable)
                            <option class="text-uppercase" value="{{ $responsable->id }}" @selected($responsable->id == $cajaChica->autorizado_por)>{{ $responsable->nombres }}
                ...
                <div class="form-group col-lg-6 col-md-6 col-xs-6 mb-4">
                    ...
                    <select id="responsable" name="responsable" class="form-control text-uppercase @error('responsable') is-invalid @enderror">
                        <option selected disabled>Seleccione</option>
                        @foreach ($responsables as $responsable)
                            <option class="text-uppercase" value="{{ $responsable->id }}" @selected($responsable->id == $cajaChica->responsable_id)>{{ $responsable->nombres }}
                                {{ $responsable->apellidos }}</option>
                        @endforeach
                    </select>
                    @error('responsable')
                ...
                <div class="form-group col-lg-6 col-md-6 col-xs-6 mb-4">
                    ...
                    <input type="text" class="form-control @error('monto') is-invalid @enderror" id="monto" name="monto" placeholder="Monto" value="{{ $cajaChica->monto }}">
                    @error('monto')
                        <div class="alert alert-danger m-2">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-row m-2">
                <div class="form-group col-12">
                    ...
                    <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion" rows="3" style="resize: none;">{{ $cajaChica->descripcion }}</textarea>
                </div>
                @error('descripcion')
                    <div class="alert alert-danger m-2">{{ $message }}</div>
                @enderror
